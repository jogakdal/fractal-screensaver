...
### 오버레이 & 시계
- **시스템 정보 오버레이**: GPU/CPU 모델, RAM, 해상도, FPS, 줌 배율, 반복 횟수 - 이중 외곽선 렌더링으로 어떤 배경에서도 가독성 확보
- **7-세그먼트 디지털 시계**: 바운스 물리 + LED 스타일 반투명 렌더링

### 통합 인스톨러
- **단일 바이너리**: 인스톨러 + 스크린세이버 통합 - 확장자 기반 모드 감지 (.exe=설치, .scr=스크린세이버)
- **AV 친화적**: RCDATA 임베딩(드로퍼 패턴) 제거, CopyFileW(자기 자신) 방식

### 가볍고 독립적
- **단일 파일** (~453 KB .exe) — 런타임 의존성, .NET, Java 불필요
