...
다운로드한 `.exe`를 실행하면 Windows가 다음과 같은 경고를 띄울 수 있습니다:

> **Windows의 PC 보호**
> Microsoft Defender SmartScreen에서 인식할 수 없는 앱의 시작을 차단했습니다.

이는 FractalSaver가 **코드 서명되어 있지 않아서** 발생합니다 (코드 서명 인증서는 연 $70~600으로 소규모 무료 프로젝트에는 부담입니다). 파일 자체는 안전하며 단지 Microsoft 평판 데이터베이스에 등록되지 않았을 뿐입니다.

**실행하려면**:
1. 경고 창에서 **"추가 정보"** 클릭
2. **"실행"** 버튼이 나타나면 클릭
3. 런처가 정상적으로 열리고 이후 동작은 동일합니다

또는 실행 전에 파일 차단 해제:
1. 다운로드한 `.exe` 우클릭 → **속성**
2. 일반 탭 하단의 **"차단 해제"** 체크 → **확인**
3. 더블클릭 — 경고 없이 바로 실행

이 작업은 다운로드한 파일마다 한 번만 하면 됩니다.
...
|------|------|--------|
| 줌 속도 | 1 (느림) ~ 10 (빠름) | 8 |
| 컬러 스타일 | 자동 순환 / Smooth / Contour / Stripe / TIA / Classic / Sculpted 3D | 자동 순환 |
| 프랙탈 렌더링 | 켜기 / 끄기 | 켜기 |
| 정보 오버레이 | 켜기 / 끄기 | 켜기 |
| 디지털 시계 | 켜기 / 끄기 | 끄기 |
